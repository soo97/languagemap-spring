...
public interface FavoritePlaceControllerDocs {
    ...
    ApiResponseDTO<Void> addFavoritePlace(@RequestBody FavoritePlaceDto.RequestAddFavoritePlace request);

    @Operation(summary = "장소 즐겨찾기 삭제", description = "사용자의 장소 즐겨찾기를 삭제한다.")
    ApiResponseDTO<Void> removeFavoritePlace(@RequestBody FavoritePlaceDto.RequestRemoveFavoritePlace request);

    @Operation(summary = "장소 즐겨찾기 목록 조회", description = "사용자의 장소 즐겨찾기 목록을 조회한다.")
    ApiResponseDTO<List<FavoritePlaceDto.ResponseFavoritePlace>> getFavoritePlaces(@RequestParam Long userId);
}
