...
public interface LearningControllerDocs {
    ...
    @Operation(
            summary = "학습 기록 저장",
            description = "사용자의 학습 기록과 점수를 저장합니다."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "사용자 또는 학습 세션을 찾을 수 없음",
                    content = @Content(
                            mediaType = "application/json",
                            examples = @ExampleObject(
                                    value = """
    ...
    ResponseEntity<ApiResponseDTO<Void>> recordStudyLog(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "학습 기록 저장 요청 값",
                    required = true,
                    content = @Content(
                            schema = @Schema(implementation = LearningLogDto.RequestCreate.class),
                            examples = @ExampleObject(
                                    value = """
                                        {
                                          "userId": 1,
            ...
            LearningLogDto.RequestCreate request
    );
